...
import { getMetaValue, loadUserMeta, setMetaValue, saveUserMeta, } from "../../../meta/store.js";
...
export const command = {
    ...
    action: async (args) => {
        ...
        const portInput = args.port ?? getMetaValue(store, "network", "port", "8888");
        const port = Number(portInput);
        if (isNaN(port) || port < 1 || port > 65535) {
            console.log(paint(color.red, `✖ Invalid port: ${portInput}. Must be between 1-65535.`));
            return;
        }
        // --- Parse host ---
        const host = String(args.host ?? getMetaValue(store, "network", "host", "127.0.0.1"));
        // --- Determine encryption mode ---
        // Commander.js sets args.encrypt = false when --no-encrypt is used
        const encrypt = (args.encrypt ?? getMetaValue(store, "network", "encrypt", true)) !==
            false;
        // --- Handle key provisioning (CRITICAL FIX: await addKey) ---
        if (args.key) {
            if (!encrypt) {
                console.log(paint(color.yellow, "⚠ Warning: --key provided but --no-encrypt specified. Key will be ignored."));
            }
            else {
                try {
                    // ✅ FIXED: Properly await asynchronous key persistence
                    await addKey(String(args.key));
                    console.log(paint(color.green, "✓ Encryption key accepted and persisted to .shuffle/e2ee.tsl3"));
                }
                catch (err) {
        ...
        if (encrypt && !(await hasValidKey())) {
            console.log(paint(color.red, "✖ Encryption enabled but no valid key set.\n" +
        ...
        if (server && !args.force) {
            ...
            console.log(paint(color.yellow, `⚠ Server already running on ${currentHost}:${currentPort}. Use --force to restart.`));
            return;
        }
        ...
        if (args.save) {
            ...
            saveUserMeta(store);
            console.log(paint(color.green, "✓ Network configuration saved to meta store"));
        }
        // --- Start server ---
        try {
            ...
            console.log(paint(color.cyan, `✓ Shuffle server active on ${host}:${port} (${securityBadge})`));
            if (!encrypt) {
                console.log(paint(color.yellow, "⚠️  WARNING: Traffic is NOT encrypted. Use --key for production deployments."));
            }
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            ...
            if (msg.includes("EADDRINUSE")) {
                console.log(paint(color.yellow, `💡 Port ${port} is in use. Try: --port ${port === 8888 ? 8889 : 8888}`));
            }
            else if (msg.includes("EACCES")) {
                console.log(paint(color.yellow, "💡 Ports <1024 require root privileges. Use port >1024 or sudo."));
            }
            else if (msg.includes("ENOENT") && encrypt) {
                console.log(paint(color.yellow, "💡 Missing key file. Provide --key '<passphrase>' to initialize encryption."));
            }
        }
    },
};
