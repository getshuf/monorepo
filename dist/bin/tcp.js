 ...
 */
export async function addKey(passphrase) {
 ...
 */
async function persistKeyMetadata(passphrase) {
    try {
        ...
        console.log(paint(color.cyan, `✓ Key metadata persisted to ${KEY_FILE} (${(FILE_SIZE / (1024 * 1024)).toFixed(1)} MiB)`));
    }
    catch (err) {
        console.error(paint(color.red, `✗ Failed to persist key metadata: ${err.message}`));
        throw err;
    }
}
/* ---------------- Shuffle Protocol ---------------- */
function shuffleHandler(conn, data) {
...
function onData(conn, raw) {
    ...
    if (encryptionKey && !conn.authenticated) {
...
export async function startServer(opts) {
    if (listening && !opts.force) {
        console.warn(paint(color.yellow, "⚠ Server already running (use force=true to restart)"));
        return;
    }
    if (listening && opts.force) {
        await stopServer();
    ...
    if (!opts.encrypt) {
        encryptionKey = null;
        console.log(paint(color.yellow, "⚠ Encryption disabled per configuration"));
    }
    ...
    server = createServer((conn) => {
        ...
        conn.on("data", (buf) => {
            try {
                ...
                console.error(paint(color.red, `✗ Protocol error: ${err.message}`));
                conn.write("fail:internal-error\n");
                conn.destroy();
            }
        });
        conn.on("timeout", () => {
            console.debug(paint(color.gray, `⏱️ Connection timeout from ${conn.remoteAddress}`));
            conn.destroy();
        });
        conn.on("error", (err) => {
        ...
        conn.on("end", () => {
            console.debug(paint(color.gray, `🔌 Connection closed from ${conn.remoteAddress}`));
        });
    });
    ...
    server.on("error", (err) => {
        if (err.code === "EADDRINUSE") {
            ...
            console.error(paint(color.red, `✗ Permission denied for port ${opts.port} (try >1024)`));
        }
        else {
    ...
    return new Promise((resolve, reject) => {
        server.listen(opts.port, opts.host, () => {
            ...
            console.log(paint(color.green, `✓ Shuffle server active on ${addr.address}:${addr.port} ` +
                `(encryption: ${opts.encrypt ? "enabled" : "disabled"})`));
            // Security notice when encryption is disabled
            if (!opts.encrypt) {
                console.warn(paint(color.yellow, "⚠ WARNING: Server running WITHOUT encryption. " +
...
export async function stopServer() {
    ...
    return new Promise((resolve) => {
        ...
        server.close((err) => {
            ...
            server = null;
            if (err) {
                console.error(paint(color.red, `✗ Error stopping server: ${err.message}`));
            }
            else {
        ...
        setTimeout(() => {
            server?.getConnections((_, count) => {
                if (count > 0) {
                    console.warn(paint(color.yellow, `⚠ Forcing closure of ${count} lingering connection(s)`));
                    server?.close();
                }
            });
